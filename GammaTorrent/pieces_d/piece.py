...
class Piece(object):
    ...
    def update_block_status(self): 
        for i, block in enumerate(self.blocks):
            if block.state == State.PARTIAL and (time.time() - block.last_seen) > 5:
                self.blocks[i] = Block()
    ...
    # fetching an empty block and converting it into partially field state
    def get_empty_block(self):
        ...
        for block_index, block in enumerate(self.blocks):
            if block.state == State.EMPTY:
                self.blocks[block_index].state = State.PARTIAL
    ...
    def are_all_blocks_full(self):
        for block in self.blocks:
            if block.state == State.EMPTY or block.state == State.PARTIAL:
                return False
