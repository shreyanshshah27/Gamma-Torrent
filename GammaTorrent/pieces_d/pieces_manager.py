...
import pieces_d.piece as piece
import bitstring
import logging
...
class PiecesManager(object):
    ...
    # checking if all the pieces are completed or not
    ...
    def _generate_pieces(self):
        # dictionary of each pieces content
        pieces = []
        last_piece = self.number_of_pieces - 1

        for i in range(self.number_of_pieces):
            # 20-byte SHA1 hash for every piece.
            start = i * 20
            end = start + 20

            if i == last_piece:
                # for the last piece, the piece-length might not be same as the piece-length of others.
